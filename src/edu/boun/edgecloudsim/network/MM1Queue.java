...
public class MM1Queue extends NetworkModel {
    ...
    */
	...
	public double getUploadDelay(int sourceDeviceId, int destDeviceId, double dataSize) {
		...
		if(destDeviceId == SimInputConfig.CLOUD_DATACENTER_ID){
		...
		else if(destDeviceId == SimInputConfig.EDGE_ORCHESTRATOR_ID){
			delay = getWlanUploadDelay(accessPointLocation, CloudSim.clock()) +
					SimInputConfig.getInstance().getInternalLanDelay();
		}
		//mobile device to edge device (wifi access point)
		else if (destDeviceId == SimInputConfig.GENERIC_EDGE_DEVICE_ID) {
			delay = getWlanUploadDelay(accessPointLocation, CloudSim.clock());
		}
    ...
    */
	...
	public double getDownloadDelay(int sourceDeviceId, int destDeviceId, double dataSize) {
		...
		if(sourceDeviceId == SimInputConfig.EDGE_ORCHESTRATOR_ID &&
				destDeviceId == SimInputConfig.GENERIC_EDGE_DEVICE_ID){
			return SimInputConfig.getInstance().getInternalLanDelay();
		}
		...
		if(sourceDeviceId == SimInputConfig.CLOUD_DATACENTER_ID){
		...
		//edge device (wifi access point) to mobile device
			...
			if(host.getLocation().getServingWlanId() != accessPointLocation.getServingWlanId())
				delay += (SimInputConfig.getInstance().getInternalLanDelay() * 2);
		}
	...
	private double getWlanDownloadDelay(Location accessPointLocation, double time) {
		return calculateMM1(0,
				SimInputConfig.getInstance().getWlanBandwidth(),
				WlanPoissonMean,
				avgTaskOutputSize,
	...
	private double getWanDownloadDelay(Location accessPointLocation, double time) {
		return calculateMM1(SimInputConfig.getInstance().getWanPropogationDelay(),
				SimInputConfig.getInstance().getWanBandwidth(),
				WanPoissonMean,
				avgTaskOutputSize,
				getDeviceCount(accessPointLocation, time));
	}
